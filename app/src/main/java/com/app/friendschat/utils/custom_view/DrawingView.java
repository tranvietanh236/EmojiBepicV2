...
public class DrawingView extends View{
    ...
    private com.app.friendschat.utils.custom_view.ActionStack mActionStack;//This is used for undo/redo, if null this means the undo and redo are disabled
    ...
    public void setUndoAndRedoEnable(boolean enabled){
        if (enabled)
            mActionStack = new com.app.friendschat.utils.custom_view.ActionStack();
        else
            mActionStack = null;
    }
     ...
     * @param bitmap1 to be used as a background image.
     */
    public void setBackgroundImage(Bitmap bitmap1) {
        Log.d("logBitmapphoto", "2.bitmap: "+bitmap1);

        privateSetBGBitmap(bitmap1);
    ...
    private void privateSetBGBitmap(Bitmap bitmap){
        ...
        if (scaleFactor != 1)//if the bitmap is larger than the view scale it down
            bitmap = com.app.friendschat.utils.custom_view.Utilities.resizeBitmap(bitmap, ((int) (bitmap.getWidth() * scaleFactor)), (int) (bitmap.getHeight() * scaleFactor));
